...
class APIController extends BaseController
{
	public function getUid(Request $request){
        ...
        $subscriber = User::where('msisdn',$msisdn)->where('app_name', 'tst_joke')->first();
        if ($subscriber) {
            return response()->json([
                    'status'=>'1',
            ...
            $subscriber = new User;
            $subscriber->app_name = 'tst_joke';
            $subscriber->msisdn = $msisdn;
            $subscriber->status = 'UNREGISTERED';
            $subscriber->channel = 'app';
           
            //$subscriber->app_id = \Config::get('constant.joke.app_id');
	...
	public function subscription(Request $request){
        ...
        $subscriber = User::where('unique_id',$request->unique_id)->where('app_name', 'tst_joke')->first();
        if ($subscriber) {
            ...
            if(array_key_exists("subscriptionStatus", $sub_resp)){
                if($sub_resp['subscriptionStatus'] == 'REGISTERED'){
                    ...
                    $applicationId = \Config::get('constant.joke.app_id');
                    $applicationPass = \Config::get('constant.joke.app_pass');
	...
	public function subscriptionCheck(Request $request){
        ...
        $user = User::where('unique_id','=',$unique_id)->where('app_name', 'tst_joke')->orderBy('created_at', 'desc')->first();
	...
	public function unsubscription(Request $request){
        ...
        $unique_id = $request->unique_id;
        $user = User::where('unique_id','=',$unique_id)->where('app_name', 'tst_joke')->first();
        if (is_null($user)) {
            return response()->json(
                    [
                        'status'=>'0',
    ...
    public function getCategories(){
        $categoryData = JokeCategory::select('id', 'title')
					->where('status', 1)
                    ->get()->toArray();
	...
	private function getSubscriptionStatus($msisdn){
        ...
        $appId = \Config::get('constant.joke.app_id');
        $app_pass = \Config::get('constant.joke.app_pass');
	...
	private function unsubscribeUser($destinationAddress){   
        ...
        $encoding = "0";
        $version =  "1.0";
        $password = \Config::get('constant.joke.app_pass');
        $binary_header = "";
        $res = $subscripSenderObj->subscribe($destinationAddress, $password, $applicationId, $action, $version, $binary_header);    
		$responseArray = json_decode($res,true);
